...
- **Stem Separation (Demucs v4)** — Right-click an audio clip → **Separate Stems** and Meta's [Demucs v4 (hybrid transformer)](https://github.com/facebookresearch/demucs) splits it into **drums / bass / other / vocals** on four new audio tracks in the same scene. Inference runs on **ONNX Runtime** via a MIT C++ port (adapted from [sevagh/demucs.onnx](https://github.com/sevagh/demucs.onnx)) in `third_party/demucs/`, with STFT/ISTFT + 7.8 s segment chunking in C++. The ~170 MB model is **downloaded on demand** (via `curl`) into `~/.yawn/models/` on first use — not bundled. Runs on a worker thread with progress toasts; **Esc** cancels. CPU-only and genuinely slow (minutes per song — Demucs was built for GPUs), so it's opt-in behind `-DYAWN_HAS_STEM_SEPARATION=ON` (default OFF; Linux x64 + Windows x64; macOS not wired). *The AI cannot hear the drums it is extracting, will wait five minutes to not-hear them, and calls this a feature.*
...
### Quality
- **Test-Driven Development** — 1347 unit & integration tests across 159+ test suites via Google Test. Was 1306 before we deleted ~80 v1-framework tests that were superseded by their fw2 counterparts. The AI counts down too sometimes — but this run, it counted up
...
| Stem Separation | ONNX Runtime + Eigen (optional, shares the prebuilt ORT — gated on `YAWN_HAS_STEM_SEPARATION`, default OFF; Linux x64 + Windows x64; runs Demucs v4, vendored MIT port in `third_party/demucs/`; ~170 MB model downloaded on demand via `curl` into `~/.yawn/models/`) |
...
├── tests/                      # 1347 unit & integration tests (Google Test, fw2-only)
│   ├── CMakeLists.txt
...
| 31. Session-record orchestration + stuck-state cure + UX fixes (v0.62) | ✅ Done | Transport Record button now orchestrates the per-track session-record gesture: armed tracks start recording into the target scene's slot, non-armed tracks launch that scene's clip if any, transport arms last. Bright red ring in the scene label marks the record-target row. AudioEngine's emitClipStateUpdates rewritten with change-detection + per-track LastEmittedClipState cache so the previously-silent Recording / Playing → Idle transitions on a clipless track actually fire the all-clear message (fixes the "track stays red/green forever" stuck-state class structurally). PianoRollPanel renderToolbar per-frame string-alloc guarded with a change-check (was throwing std::bad_alloc on long idle sessions after the heap fragmented). Release / RelWithDebInfo builds link with `/SUBSYSTEM:WINDOWS /ENTRY:mainCRTStartup` so YAWN.exe no longer spawns a stray console window alongside the main window |
| 32. Automation arming + clear scopes + arrangement bugs + SplineEQ persistence (v0.63) | ✅ Done | Global automation-record arm above the per-track AutoMode (AUTO button on transport, persisted with the project). "Clear Clip Automation" / "Clear Track Automation" / "Clear All Automation…" menus across three right-click scopes + the Edit menu, all undoable. `Track::midiOverdub` field + serialization. Arrangement-view fixes: loop marker drag (added missing `captureMouse()`, bumped hit-tol to 8 px, end-marker priority when both at the same x), edge auto-scroll while dragging markers or scrubbing the playhead, `+`/`=`/`-` keyboard zoom centred on the playhead, "Loop Region" export option visible whenever any non-empty loop range exists. PianoRollPanel UAF guard against slot mutations destroying its held `MidiClip*`. SplineEQ persistence — three-layer bug: name-collision-on-the-five-shared-keys (fixed via per-node-keyed `saveExtraState/loadExtraState`), missing extra-state hooks in `DevicePresetHelpers::saveDevicePreset(AudioEffect&)`, and the `chain.append → init() → wipes restored params` ordering bug (fixed by appending first then operating on the slot pointer). Regression test pins the round-trip |
| 33. Auto-arm UX completion (v0.64) | ✅ Done | Scene-label click runs the same per-track orchestration the Record button does — armed tracks start recording into that scene, non-armed tracks launch the slot's clip OR stop the track if empty (Live-style "Session Take" gesture). Per-track MIDI Overdub toggle exposed on the track-header right-click menu. Per-clip Auto-Rec disable on the session-clip right-click — wired all the way through `SetClipAutoRecordDisabledMsg` + the LaunchClipMsg.autoRecordDisabled side channel so the engine sees it on both initial launch and live-toggle. Visual A–H knobs gain `setOnKnobTouch` → `AutoParamTouchMsg{TargetType::VisualKnob}` so global-arm + per-track Touch/Latch on a visual track actually records visual-param breakpoints |
