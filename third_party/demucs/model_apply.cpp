    ...
    demucsonnx::ProgressCallback cb)
{
    ...
    for (int segment_offset = 0; segment_offset < shifted_length; segment_offset += stride_samples)
    {
        ...
        // Update progress — a false return signals cancellation, so bail
        // out early with an empty tensor (yawn's Cancel button).
        if (!cb(inference_progress + increment_per_chunk, "Segment inference complete"))
            return Eigen::Tensor3dXf();
