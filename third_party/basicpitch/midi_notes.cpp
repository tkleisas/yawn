...
// ── Public wrappers (added for yawn integration) ──────────────────────
// Expose note creation + frame timing without the libremidi MIDI-file
// serialization from the original basicpitch.cpp (yawn builds a MidiClip
// directly from the NoteEvents).
namespace basic_pitch {

std::vector<NoteEvent> notes_from_inference(const InferenceResult &result,
                                            bool use_melodia_trick,
                                            bool include_pitch_bends)
{
    std::vector<NoteEvent> notes = output_to_notes_polyphonic(
        result, use_melodia_trick, include_pitch_bends);
    if (include_pitch_bends)
        drop_overlapping_pitch_bends(notes);
    return notes;
}
