...
TEST_F(MidiClipEngineTest, MultipleTracksIndependent) {
    ...
    m_engine.scheduleClip(0, &clipA);
    m_engine.scheduleClip(1, &clipB);
    m_engine.checkAndFirePending();
    m_transport.play();
...
TEST_F(MidiClipEngineTest, OutOfRangeTrackIndexIgnored) {
    MidiClip clip;
    m_engine.scheduleClip(-1, &clip);
    m_engine.scheduleClip(kMaxTracks, &clip);
