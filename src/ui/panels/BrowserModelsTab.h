...
// Double-click assigns the model to the selected visual clip. Each row
// shows a live rendered thumbnail (looked up via the thumbnail callback;
// generated by the engine at frame-start); a generic glyph stands in
// until the thumbnail is ready.
...
public:
    ...
    // Render-time lookup: path → cached thumbnail GL texture (0 if none
    // yet). Generation happens elsewhere (frame-start); this only reads.
    void setThumbnailLookup(std::function<unsigned(const std::string&)> cb) {
        m_thumbLookup = std::move(cb);
    }

    // Library entries (for the App's frame-start thumbnail-generation pass).
    const std::vector<visual::ModelLibraryEntry>& entries() const {
...
public:
    void render(UIContext& ctx) override {
        ...
        for (int i = 0; i < visible + 1 && (i + m_scrollOffset) < total; ++i) {
            ...
            // Thumbnail (rendered) if available, else a generic 3D glyph.
            const float ih = kRowHeight - 6;
            const float iw = ih * 4.0f / 3.0f;     // 4:3 like the render FBO
            const float ix = x + 5, iy = ry + 3;
            unsigned tex = m_thumbLookup ? m_thumbLookup(m.path) : 0;
            if (tex) {
                r.drawTexturedQuad(ix, iy, iw, ih, 0.0f, 0.0f, 1.0f, 1.0f,
                                   Color{255, 255, 255, 255}, tex);
            } else {
                r.drawRectOutline(ix, iy, iw, ih, Color{120, 150, 200, 255}, 1.0f);
                r.drawRect(ix + iw * 0.3f, iy + ih * 0.25f, iw * 0.4f, ih * 0.5f,
                           Color{70, 90, 130, 200});
            }

            float textY = ry + (kRowHeight - lh) * 0.5f;
            tm.drawText(r, m.name, ix + iw + 6, textY, fs,
...
private:
    static constexpr float kRowHeight = 40.0f;   // tall enough to show a thumbnail
    ...
    std::function<unsigned(const std::string&)> m_thumbLookup;
};
